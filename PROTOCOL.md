...
A scripture verse longer than ~200 characters, or a song slide with more than 4 lines, is automatically split client-side into parts (`splitTextIntoParts`/`splitLinesIntoParts` in `public/control/app.js`) so it never has to be shrunk down to illegibility on the projector. Next/Prev steps through the parts of the *current* verse/slide first, only advancing to an actual different verse/slide once off the start/end of the parts (state tracked in `currentScripture.parts`/`.partIndex` and `currentSlideParts`/`currentSlidePartIndex`). This is entirely `/control`-local, same as staging - the server and `/display` just see a normal `reference`/`slideLabel` string that happens to end in `" (N/M)"` when split, and render it as ordinary text with no special handling.
...
- `GET /api/bible/search?q=<text>&translations=kjv,web` → instant results for **offline** translations (reference parse like `"jn 3:16"`, `"john 3:16-18"`, or keyword search like `"god so loved"`). Returns `{ "results": [{ "translation": "kjv", "book": "John", "chapter": 3, "verse": 16, "text": "..." }], "bookMatch": "Joshua" | null }`. Must return in well under 200ms for the whole offline Bible. Keyword search boosts matches in a book whose name the query itself resolves to (e.g. `"joshua"`) above same-tier matches elsewhere (e.g. the person Joshua mentioned in Exodus) - otherwise canonical book order alone decided the tie, not relevance. `bookMatch` is set the instant the query text unambiguously names exactly one book (a complete known abbreviation like `"jn"`, or a partial full name like `"jos"` that only "Joshua" starts with) - `/control` uses it to auto-stage that book's chapter 1 verse 1 for speed, stopping naturally once more of a real reference is typed (e.g. `"joshua 3"` no longer just names a bare book).
